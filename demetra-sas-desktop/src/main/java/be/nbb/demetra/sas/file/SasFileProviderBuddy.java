 ...
 * @since 2.2.0
 */
@ServiceProvider(service = IDataSourceProviderBuddy.class, supersedes = "be.nbb.demetra.sas.SasProviderBuddy")
public final class SasFileProviderBuddy implements IDataSourceProviderBuddy {

    private final ConcurrentMap autoCompletionCache;

    public SasFileProviderBuddy() {
        this.autoCompletionCache = GuavaCaches.ttlCacheAsMap(Duration.ofMinutes(1));
    }

    @Override
    public String getProviderName() {
        return SasFileProvider.NAME;
    }
    ...
    public boolean editBean(String title, Object bean) throws IntrospectionException {
        if (bean instanceof SasFileBean) {
            Optional<SasFileProvider> provider = lookupProvider();
            if (provider.isPresent()) {
                return new PropertySheetDialogBuilder()
                        .title(title)
                        .icon(getIcon(BeanInfo.ICON_COLOR_16x16, false))
                        .editSheet(createSheet((SasFileBean) bean, provider.get(), autoCompletionCache));
            }
        }
    ...
    private static Optional<SasFileProvider> lookupProvider() {
        return TsProviders.lookup(SasFileProvider.class, SasFileProvider.NAME).toJavaUtil();
    }
    ...
    private static Sheet createSheet(SasFileBean bean, SasFileProvider loader, ConcurrentMap cache) {
    ...
    private static NodePropertySetBuilder withSource(NodePropertySetBuilder b, SasFileBean bean, IFileLoader loader, ConcurrentMap cache) {
        ...
        b.withAutoCompletion()
                .select(bean, "table")
                .source(SasAutoCompletion.onTables(loader, bean::getFile, cache))
    ...
    private static NodePropertySetBuilder withStructure(NodePropertySetBuilder b, SasFileBean bean, SasFileProvider loader, ConcurrentMap cache) {
        AutoCompletionSource columnCompletion = SasAutoCompletion.onColumns(loader.getSasquatch(), loader, bean::getFile, bean::getTable, cache);
        ListCellRenderer columnRenderer = new SasColumnRenderer();
        b.withAutoCompletion()
                ...
                .separator(",")
                .defaultValueSupplier(() -> SasAutoCompletion.getDefaultColumnsAsString(loader.getSasquatch(), loader, bean::getFile, bean::getTable, cache, ","))
    ...
    private static NodePropertySetBuilder withOptions(NodePropertySetBuilder b, SasFileBean bean) {
    ...
    private static NodePropertySetBuilder withCache(NodePropertySetBuilder b, SasFileBean bean) {
        b.withInt()
